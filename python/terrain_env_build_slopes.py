...
terrain_ori = terrain_and_contact_seq["TerrainEulerY"]
...
with open(world_file_path, 'a') as f:
    for surf_idx in range(len(terrain_model)):
        ...
        temp_surf_eulerY = terrain_ori[surf_idx]
        print(temp_surf)
        ...
        temp_center_z = max(temp_surf[0:,2])/2.0
        size_z = 0.01
        print("size z ", size_z)
        # print(temp_type)
        f.write('    <!-- Place a Block -->\n')
        f.write('      <model name = "block_'+ str(surf_idx)+'"' + '>\n')
        f.write('        <pose> ' + str(temp_center_x) + " " + str(temp_center_y) + " " + str(temp_center_z) + ' 0.0 ' + str(temp_surf_eulerY) + ' 0.0' + '</pose>\n')
        f.write('        <static>'+ str(1) +'</static>\n')
